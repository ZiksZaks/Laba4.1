...
if (hero.dx != 0) {
    ...
    if (hero.dx > 0) { // Движение вправо
        for (int y = top_tile; y <= bottom_tile; y++) {
            if (TileMap[y][right_tile] == 'B') {
                isWallHitX = true;
                hero.x = right_tile * tile_size - hero.HERO_WIDTH; // Убираем смещение
                break;
            }
        }
    } else { // Движение влево
        for (int y = top_tile; y <= bottom_tile; y++) {
            if (TileMap[y][left_tile] == 'B') {
                isWallHitX = true;
                hero.x = (left_tile + 1) * tile_size; // Убираем смещение
                break;
            }
        }
    }
    ...
    if (!isWallHitX) {
        hero.x += hero.dx; // Обновляем позицию только если нет коллизии
    }
    hero.dx = 0; // Сбрасываем скорость после проверки
}
...
if (hero.dy != 0) {
    ...
    if (hero.dy > 0) { // Движение вниз
        for (int x = left_tile; x <= right_tile; x++) {
            if (TileMap[bottom_tile][x] == 'B') {
                isWallHitY = true;
                hero.y = (bottom_tile * tile_size) - hero.HERO_HEIGHT; // Убираем смещение
                hero.isAirborne = false; // Персонаж на земле
                break;
            }
        }
    } else { // Движение вверх
        for (int x = left_tile; x <= right_tile; x++) {
            if (TileMap[top_tile][x] == 'B') {
                isWallHitY = true;
                hero.y = (top_tile + 1) * tile_size; // Убираем смещение
                break;
            }
        }
    }
    ...
    if (!isWallHitY) {
        hero.y += hero.dy; // Обновляем позицию только если нет коллизии
        hero.isAirborne = true; // Если нет коллизии, персонаж в воздухе
    }
    hero.dy = 0; // Сбрасываем скорость после проверки
}
...
void Hero_Move(Hero *obj) {
    ...
    UpdateHeroPositionAndCollisions(obj, 1.0f); // deltaTime ����� �������� �� �������� ����� ����� �������
}
